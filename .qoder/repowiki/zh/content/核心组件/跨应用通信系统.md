...
## 更新摘要
**变更内容**
- 新增动态Origin模式匹配系统，从静态allow-list升级为正则表达式模式匹配
- 引入localhost匹配器和域名后缀匹配器，支持灵活的跨域安全校验
- 改进的origin验证逻辑，包括动态添加云IDE环境origin
- 增强跨域安全校验的灵活性和可靠性
...
**更新** 系统现已支持动态Origin模式匹配系统，包括localhost匹配器和域名后缀匹配器，从静态allow-list升级为正则表达式模式匹配，大幅提升了跨域通信的安全性和灵活性。
...
graph TB
subgraph "主应用"
M_Bridge["Bridge<br/>动态Origin模式匹配<br/>消息监听/路由/广播"]
Router["路由<br/>用于跳转"]
Store["用户状态存储<br/>用于令牌获取"]
IframeLoader["IframeLoader<br/>iframe管理器"]
end
subgraph "子应用"
I_Bridge["IframeBridge<br/>动态Origin模式匹配<br/>消息监听/处理"]
...
**图表来源**
- [packages/main-app/src/core/bridge.js](file://packages/main-app/src/core/bridge.js#L1-L258)
- [packages/iframe-sub-app/src/bridge.js](file://packages/iframe-sub-app/src/bridge.js#L1-L233)
- [packages/main-app/src/core/iframeLoader.js](file://packages/main-app/src/core/iframeLoader.js#L1-L334)
- [user-docs/api/bridge.md](file://user-docs/api/bridge.md#L1-L151)
...
## 核心组件
- **主应用 Bridge**：负责消息监听、默认处理器注册、消息分发、广播、令牌同步、路由跳转等。现已支持动态Origin模式匹配，包括localhost匹配器和域名后缀匹配器。
- **Iframe 子应用 IframeBridge**：负责与主应用通信的消息监听、默认处理器注册、令牌请求/同步、心跳、高度上报、导航等。具备智能的Origin模式匹配能力和动态origin列表管理。
- **IframeLoader**：新增的iframe管理器，负责iframe实例的生命周期管理、高度自适应和通信协调。
- **文档**：提供 API 说明、内置消息类型、使用示例与端口配置。

**章节来源**
- [packages/main-app/src/core/bridge.js](file://packages/main-app/src/core/bridge.js#L9-L40)
- [packages/iframe-sub-app/src/bridge.js](file://packages/iframe-sub-app/src/bridge.js#L5-L30)
- [packages/main-app/src/core/iframeLoader.js](file://packages/main-app/src/core/iframeLoader.js#L9-L39)
- [user-docs/api/bridge.md](file://user-docs/api/bridge.md#L1-L151)

## 架构总览
系统以 postMessage 为核心通信协议，主应用与子应用之间通过消息类型进行路由与处理。主应用维护允许的 origin 白名单并进行跨域校验；子应用同样维护白名单并向上游发送消息。**更新** 现在支持动态Origin模式匹配系统，通过正则表达式模式匹配机制自动维护origin白名单。
...
Note over Main,Child : "动态Origin模式匹配与校验"
Note over Loader,Child : "iframe生命周期管理"
```

**图表来源**
- [packages/main-app/src/core/bridge.js](file://packages/main-app/src/core/bridge.js#L109-L131)
- [packages/iframe-sub-app/src/bridge.js](file://packages/iframe-sub-app/src/bridge.js#L108-L132)
...
-originPatterns : RegExp[] // 动态Origin模式匹配器
-handlers : Map
-isListening : boolean
+constructor()
...
**图表来源**
- [packages/main-app/src/core/bridge.js](file://packages/main-app/src/core/bridge.js#L9-L234)

**章节来源**
- [packages/main-app/src/core/bridge.js](file://packages/main-app/src/core/bridge.js#L11-L17)
- [packages/main-app/src/core/bridge.js](file://packages/main-app/src/core/bridge.js#L19-L25)
- [packages/main-app/src/core/bridge.js](file://packages/main-app/src/core/bridge.js#L27-L30)
- [packages/main-app/src/core/bridge.js](file://packages/main-app/src/core/bridge.js#L38-L91)
- [packages/main-app/src/core/bridge.js](file://packages/main-app/src/core/bridge.js#L109-L131)
- [packages/main-app/src/core/bridge.js](file://packages/main-app/src/core/bridge.js#L156-L172)
- [packages/main-app/src/core/bridge.js](file://packages/main-app/src/core/bridge.js#L178-L188)
- [packages/main-app/src/core/bridge.js](file://packages/main-app/src/core/bridge.js#L194-L199)
- [packages/main-app/src/core/bridge.js](file://packages/main-app/src/core/bridge.js#L208-L222)
- [packages/main-app/src/core/bridge.js](file://packages/main-app/src/core/bridge.js#L227-L234)

### Iframe 子应用 IframeBridge 组件
- **职责**
  - **动态Origin模式匹配**：支持localhost匹配器和域名后缀匹配器，确保通信安全。
...
- **关键实现点**
  - **Origin模式匹配**：使用originPatterns数组进行动态匹配，支持localhost和域名后缀匹配。
  - 跨域校验：支持静态origin列表、动态Origin模式匹配和同源检测。
...
**图表来源**
- [packages/iframe-sub-app/src/bridge.js](file://packages/iframe-sub-app/src/bridge.js#L5-L227)

**章节来源**
- [packages/iframe-sub-app/src/bridge.js](file://packages/iframe-sub-app/src/bridge.js#L7-L20)
- [packages/iframe-sub-app/src/bridge.js](file://packages/iframe-sub-app/src/bridge.js#L35-L38)
- [packages/iframe-sub-app/src/bridge.js](file://packages/iframe-sub-app/src/bridge.js#L43-L103)
- [packages/iframe-sub-app/src/bridge.js](file://packages/iframe-sub-app/src/bridge.js#L108-L132)
- [packages/iframe-sub-app/src/bridge.js](file://packages/iframe-sub-app/src/bridge.js#L144-L148)
- [packages/iframe-sub-app/src/bridge.js](file://packages/iframe-sub-app/src/bridge.js#L153-L158)
- [packages/iframe-sub-app/src/bridge.js](file://packages/iframe-sub-app/src/bridge.js#L163-L172)
- [packages/iframe-sub-app/src/bridge.js](file://packages/iframe-sub-app/src/bridge.js#L177-L192)
- [packages/iframe-sub-app/src/bridge.js](file://packages/iframe-sub-app/src/bridge.js#L197-L215)
- [packages/iframe-sub-app/src/bridge.js](file://packages/iframe-sub-app/src/bridge.js#L220-L226)
...
**章节来源**
- [packages/main-app/src/core/bridge.js](file://packages/main-app/src/core/bridge.js#L167-L172)
- [packages/main-app/src/core/bridge.js](file://packages/main-app/src/core/bridge.js#L178-L188)
- [packages/main-app/src/core/bridge.js](file://packages/main-app/src/core/bridge.js#L208-L222)
...
**章节来源**
- [user-docs/api/bridge.md](file://user-docs/api/bridge.md#L136-L151)
- [packages/main-app/src/core/bridge.js](file://packages/main-app/src/core/bridge.js#L47-L91)
- [packages/iframe-sub-app/src/bridge.js](file://packages/iframe-sub-app/src/bridge.js#L45-L103)

### 动态Origin模式匹配系统
**新增** 系统的核心安全特性，从静态allow-list升级为正则表达式模式匹配，大幅提升了跨域通信的安全性和灵活性。

- **主应用动态Origin模式匹配**
  - **localhost匹配器**：`/^https?:\/\/localhost(:\d+)?$/` 支持http和https协议的localhost匹配。
  - **域名后缀匹配器**：`/\.hon\.ide\.dev\.jh/` 支持特定域名后缀的匹配。
  - **动态origin添加**：自动检测并添加云IDE环境的origin到allowedOrigins列表。
  - **跨域安全校验**：支持静态origin列表、同源检测和动态Origin模式匹配。
- **子应用动态Origin模式匹配**
  - **Origin模式匹配器**：与主应用相同的localhost匹配器和域名后缀匹配器。
  - **Set去重机制**：使用 `[...new Set([...this.allowedOrigins, ...payload.allowedOrigins])]` 确保origin唯一性。
  - **跨域安全校验**：支持静态origin列表、动态Origin模式匹配和同源检测。
- **跨域安全校验**
  - 接收消息时校验 event.origin 是否在白名单内、与当前窗口同源或匹配动态模式。
...
- [packages/main-app/src/core/bridge.js](file://packages/main-app/src/core/bridge.js#L19-L25)
- [packages/main-app/src/core/bridge.js](file://packages/main-app/src/core/bridge.js#L27-L30)
- [packages/main-app/src/core/bridge.js](file://packages/main-app/src/core/bridge.js#L110-L118)
- [packages/iframe-sub-app/src/bridge.js](file://packages/iframe-sub-app/src/bridge.js#L7-L20)
- [packages/iframe-sub-app/src/bridge.js](file://packages/iframe-sub-app/src/bridge.js#L51-L54)
- [packages/iframe-sub-app/src/bridge.js](file://packages/iframe-sub-app/src/bridge.js#L109-L116)
...
- **动态Origin匹配异常**：localhost匹配器和域名后缀匹配器失败时回退到基础白名单。
- **销毁阶段**：移除消息监听与清理处理器，防止内存泄漏。

**章节来源**
- [packages/main-app/src/core/bridge.js](file://packages/main-app/src/core/bridge.js#L115-L118)
- [packages/main-app/src/core/bridge.js](file://packages/main-app/src/core/bridge.js#L167-L172)
- [packages/iframe-sub-app/src/bridge.js](file://packages/iframe-sub-app/src/bridge.js#L113-L116)
- [packages/iframe-sub-app/src/bridge.js](file://packages/iframe-sub-app/src/bridge.js#L227-L233)
...
- **性能优化建议**
  ...
  - 在销毁前移除监听，避免重复绑定。
  - **动态Origin模式匹配**：使用originPatterns数组进行高效模式匹配。
...
- **子应用侧**
  - **动态Origin模式匹配**：等待主应用下发 INIT 消息，自动更新allowedOrigins列表。
...
- **最佳实践**
  ...
  - **动态Origin模式匹配**：充分利用localhost匹配器和域名后缀匹配器功能，减少手动配置。
...
- [packages/iframe-sub-app/src/bridge.js](file://packages/iframe-sub-app/src/bridge.js#L45-L67)
- [packages/iframe-sub-app/src/bridge.js](file://packages/iframe-sub-app/src/bridge.js#L108-L132)
...
- **子应用 IframeBridge 依赖**
  ...
  - DOM：用于高度计算与页面元素更新。
  - **动态Origin模式匹配**：依赖originPatterns数组和正则表达式匹配。
- **IframeLoader 依赖**
...
I_Bridge --> DOM["DOM"]
I_Bridge --> DynamicOrigin["动态Origin模式匹配"]
...
- [packages/iframe-sub-app/src/bridge.js](file://packages/iframe-sub-app/src/bridge.js#L14-L20)
...
- **跨域校验**
  ...
  - **动态Origin模式匹配**：使用originPatterns数组进行高效模式匹配，支持localhost和域名后缀匹配。
- **IframeLoader 性能**
...
**章节来源**
- [packages/main-app/src/core/bridge.js](file://packages/main-app/src/core/bridge.js#L110-L118)
- [packages/iframe-sub-app/src/bridge.js](file://packages/iframe-sub-app/src/bridge.js#L110-L111)
- [packages/main-app/src/core/iframeLoader.js](file://packages/main-app/src/core/iframeLoader.js#L17-L18)

## 故障排除指南
- **问题：消息被拒绝**
  - 可能原因：event.origin 不在白名单、与当前窗口不同源且不匹配动态模式。
  - 处理方式：检查 allowedOrigins 配置，确认子应用 iframe 的 src 与主应用端口一致。
  - **动态Origin匹配问题**：检查localhost匹配器和域名后缀匹配器是否正确配置。
- **问题：sendToIframe() 无响应**
...
- **问题：动态Origin模式匹配失败**
  - 可能原因：localhost匹配器或域名后缀匹配器配置错误；正则表达式语法不正确。
  - 处理方式：检查正则表达式语法；确认目标origin符合匹配规则。
- **问题：Set去重机制异常**
...
## 结论
本跨应用通信系统以 postMessage 为基础，结合主应用与子应用两端的 Bridge 实现，提供了完善的跨域通信、消息路由、令牌同步与导航能力。**更新** 系统现已支持动态Origin模式匹配系统，包括localhost匹配器和域名后缀匹配器，从静态allow-list升级为正则表达式模式匹配，大幅提升了系统的安全性和灵活性。

通过严格的跨域校验与默认处理器机制，系统在保证安全性的同时，提供了良好的扩展性与易用性。新增的动态Origin模式匹配功能使得系统能够自动适应复杂的跨域场景，localhost匹配器和域名后缀匹配器确保了origin验证的准确性和灵活性。

建议在实际使用中遵循消息类型规范、进行性能优化与异常处理，并结合文档提供的示例与最佳实践快速落地。充分利用动态Origin模式匹配功能，减少手动配置，提升系统的自动化程度和安全性。
...
- **动态Origin模式匹配特性**
  - localhost匹配器：`/^https?:\/\/localhost(:\d+)?$/`
  - 域名后缀匹配器：`/\.hon\.ide\.dev\.jh/`
  - 动态origin添加：自动检测并添加云IDE环境origin
...
- [packages/iframe-sub-app/src/bridge.js](file://packages/iframe-sub-app/src/bridge.js#L15-L20)
- [packages/main-app/src/core/bridge.js](file://packages/main-app/src/core/bridge.js#L19-L25)
- [packages/main-app/src/core/iframeLoader.js](file://packages/main-app/src/core/iframeLoader.js#L31-L119)