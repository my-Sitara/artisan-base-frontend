...
## 简介
本项目为企业级微前端基础平台脚手架，采用 Monorepo 架构，基于 qiankun 的 loadMicroApp 模式实现主应用对多类子应用（Vue3/Vue2/iframe/link）的统一加载与管理。项目提供完善的布局编排系统、跨应用通信桥、状态管理与持久化、iframe 跨域治理、多应用实例同屏加载、以及预加载、心跳检测、动态上下线等高级能力，满足复杂业务场景下的微前端工程化落地。
...
- 多应用实例同屏：通过路由与容器分离支持多应用同屏，注意容器与路由的解耦设计。
