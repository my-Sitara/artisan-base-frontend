...
**更新** 通信架构现已包含动态原点管理和增强的CORS兼容性，确保跨应用通信的安全性和兼容性，支持运行时动态添加允许的源，提供更灵活的跨域通信解决方案。
...
graph TB
subgraph "主应用"
MA_Bridge["Bridge<br/>动态原点管理<br/>消息监听/处理器/广播/心跳"]
...
Iframe_App["iframe 子应用<br/>IframeBridge<br/>动态origin管理"]
end
...
## 核心组件
- Bridge（主应用侧）：统一消息监听、处理器注册、跨域校验、广播、心跳、路由跳转、Token 同步、高度上报等。**新增动态原点管理功能**。
- IframeBridge（iframe 子应用侧）：与主应用通过 postMessage 通信，支持初始化、心跳、高度上报、导航、Token 请求/同步等。**增强origin白名单管理**。
...
- 主应用桥负责接收消息、路由跳转、广播、心跳与高度适配。**支持动态原点管理**。
- 子应用桥负责发送消息、请求 Token、上报高度、心跳响应与跨应用导航。**支持运行时origin白名单更新**。
- qiankun 子应用通过 props 注入 bridge 对象，iframe 子应用通过 window.parent/postMessage 通信。
- 跨域安全通过 origin 白名单校验与 sandbox 限制保障。**增强CORS兼容性**。
...
Note over IframeBridge : "动态origin管理<br/>运行时更新allowedOrigins"
```
...
### Bridge 组件（主应用侧）
- 功能要点
  - 消息监听与跨域校验：仅接受白名单 origin 或同源消息。**支持动态origin管理**。
...
**章节来源**
- [packages/main-app/src/core/bridge.js](file://packages/main-app/src/core/bridge.js#L10-L32)
...
**更新** 新增的动态原点管理和增强的CORS兼容性进一步提升了系统的灵活性和安全性，支持运行时动态添加允许的源，提供更完善的跨域通信解决方案。建议在生产环境中完善域名白名单、消息幂等与错误恢复策略，持续优化心跳与上报频率，提升整体稳定性与性能。
...
- [user-docs/api/bridge.md](file://user-docs/api/bridge.md#L1-L150)
- [user-docs/guide/iframe-governance.md](file://user-docs/guide/iframe-governance.md#L1-L138)
- [user-docs/guide/deployment.md](file://user-docs/guide/deployment.md#L1-L90)