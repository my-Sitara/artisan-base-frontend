...
## 更新摘要
**变更内容**
- 更新了开发服务器配置优化部分，反映Iframe子应用的独立开发服务器配置
- 新增了端口管理和跨域配置的最佳实践
- 补充了开发环境下的性能优化建议
...
Iframe子应用的开发服务器配置经过专门优化，提供了更好的开发体验和性能表现。

### 独立端口配置

Iframe子应用使用独立的开发端口配置，确保与其他子应用的开发环境互不干扰：

```mermaid
graph LR
subgraph "开发端口分配"
MainApp[主应用: 8080]
Vue3App[Vue3子应用: 7080]
Vue2App[Vue2子应用: 3000]
IframeApp[Iframe子应用: 9080]
end
MainApp -.->|端口冲突| Vue3App
Vue3App -.->|端口冲突| Vue2App
Vue2App -.->|端口冲突| IframeApp
IframeApp -.->|端口冲突| MainApp
```
...
Iframe子应用的Vite配置针对开发体验进行了专门优化：

```javascript
// packages/iframe-sub-app/vite.config.js
...
开发服务器配置包含了完整的跨域支持，确保与主应用和其他子应用的通信顺畅：

1. **CORS启用**：`cors: true`允许跨域请求
2. **通配符Origin**：`Access-Control-Allow-Origin: '*'`支持任意来源
3. **开发友好**：简化了开发环境下的跨域配置

### 开发环境特性

Iframe子应用开发服务器具备以下特性：
...
**章节来源**
- [vite.config.js](file://packages/iframe-sub-app/vite.config.js#L1-L26)
- [package.json](file://packages/iframe-sub-app/package.json#L6-L10)
- [README.md](file://README.md#L17-L24)
