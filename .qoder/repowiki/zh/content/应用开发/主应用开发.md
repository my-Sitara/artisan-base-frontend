...
## 更新摘要
**所做更改**
- 更新依赖版本信息，反映现代化工具链升级：Vue 3.5.29、Vue Router 5.0.3、Element Plus 2.13.2、Vite 7.3.1、TypeScript 5.9.3、Vue TSC 2.2.0、Pinia 3.x
...
- 新增 Pinia 3.x 状态管理配置，包含持久化插件和存储模块
...
**更新** 本版本反映了现代化工具链的重大升级，包括 Vue 3.5.29、Vue Router 5.0.3、Element Plus 2.13.2、Vite 7.3.1、Pinia 3.x 等最新版本，显著提升了框架性能、类型安全性和开发体验。开发服务器经过优化，提供了更好的开发体验和跨域支持。Pinia 3.x 状态管理配合持久化插件，提供了更强大的状态管理能力。
...
- src/stores/*：状态管理（Pinia 3.x），提供应用状态、用户状态等持久化存储
...
O["类型声明"] --> P["env.d.ts"]
Q["Pinia 3.x 状态管理"] --> R["持久化插件"]
S["应用存储"] --> T["useAppStore"]
U["用户存储"] --> V["useUserStore"]
```

**图表来源**
- [packages/main-app/src/main.js:1-50](file://packages/main-app/src/main.js#L1-L50)
- [packages/main-app/src/router/index.js:1-155](file://packages/main-app/src/router/index.js#L1-L155)
- [packages/main-app/src/stores/index.js:1-11](file://packages/main-app/src/stores/index.js#L1-L11)
...
- [packages/main-app/tsconfig.json:1-46](file://packages/main-app/tsconfig.json#L1-L46)
- [packages/main-app/.eslintrc.cjs:1-56](file://packages/main-app/.eslintrc.cjs#L1-L56)
- [packages/main-app/src/env.d.ts:1-29](file://packages/main-app/src/env.d.ts#L1-L29)

**章节来源**
- [packages/main-app/package.json:1-37](file://packages/main-app/package.json#L1-L37)
- [packages/main-app/vite.config.js:1-46](file://packages/main-app/vite.config.js#L1-L46)
...
- 状态管理
  - 使用 Pinia 3.x 并启用持久化插件，导出应用、用户等 Store 供业务模块使用
- 微应用配置
...
**图表来源**
- [packages/main-app/src/router/index.js:112-140](file://packages/main-app/src/router/index.js#L112-L140)
...
- 数据源模式
  - 支持 mock 和 api 两种数据源模式，通过环境变量控制
...
- [packages/main-app/src/core/microAppManager.js:396-407](file://packages/main-app/src/core/microAppManager.js#L396-L407)
- [packages/main-app/src/core/microAppManager.js:424-447](file://packages/main-app/src/core/microAppManager.js#L424-L447)

### Pinia 3.x 状态管理
- 状态管理架构
  - 使用 Pinia 3.x 提供响应式状态管理，配合持久化插件实现数据持久化
  - 导出多个 Store：useAppStore、useUserStore
- 应用状态管理
  - useAppStore：管理当前激活应用、侧边栏状态、全局 loading 状态、微应用列表
  - 支持应用状态更新、上下线控制、应用列表刷新等功能
- 用户状态管理
  - useUserStore：管理用户 token、用户信息、登录状态
  - 支持登录登出、token 刷新、广播通知等功能

**章节来源**
- [packages/main-app/src/stores/index.js:1-11](file://packages/main-app/src/stores/index.js#L1-L11)
- [packages/main-app/src/stores/app.js:1-143](file://packages/main-app/src/stores/app.js#L1-L143)
- [packages/main-app/src/stores/user.js:1-73](file://packages/main-app/src/stores/user.js#L1-L73)

## 开发工具链

### 现代化工具链升级
**更新** 主应用已升级到最新的现代化工具链组合，带来更好的性能和开发体验：

- **Vue 3.5.29**：最新版本，提供更好的响应式系统和编译器优化
- **Vue Router 5.0.3**：支持 Composition API 更好的集成和性能提升
- **Element Plus 2.13.2**：最新 UI 组件库，包含更多现代化组件和改进的样式系统
- **Vite 7.3.1**：更快的开发服务器和构建性能，支持最新的 ES 模块特性
- **TypeScript 5.9.3**：增强的类型检查和更好的编译性能
- **Vue TSC 2.2.0**：专门针对 Vue 3 的类型检查工具，提供更好的 Vue 组件类型推断
- **Pinia 3.x**：最新状态管理库，提供更好的 TypeScript 支持和性能优化
...
**章节来源**
- [packages/main-app/tsconfig.json:1-46](file://packages/main-app/tsconfig.json#L1-L46)
- [packages/main-app/tsconfig.node.json:1-12](file://packages/main-app/tsconfig.node.json#L1-L12)
- [packages/main-app/src/env.d.ts:1-29](file://packages/main-app/src/env.d.ts#L1-L29)
...
**章节来源**
- [packages/main-app/vite.config.js:12-24](file://packages/main-app/vite.config.js#L12-L24)
- [packages/main-app/vite.config.js:25-31](file://packages/main-app/vite.config.js#L25-L31)
...
- Pinia 状态管理
  - 使用 Composition API 风格的 Store 定义
  - 启用持久化插件，配置需要持久化的状态路径

**章节来源**
- [packages/main-app/package.json:6-13](file://packages/main-app/package.json#L6-L13)
- [packages/main-app/vite.config.js:32-45](file://packages/main-app/vite.config.js#L32-L45)
- [packages/main-app/src/router/index.js:112-140](file://packages/main-app/src/router/index.js#L112-L140)

## 依赖关系分析
- 外部依赖
  - Vue3、Vue Router、Pinia 3.x、Element Plus、qiankun、axios、lodash-es 等
- 内部依赖
...
- **新增** 现代化工具链依赖
  - Vue 3.5.29、Vue Router 5.0.3、Element Plus 2.13.2、Vite 7.3.1、TypeScript 5.9.3、Vue TSC 2.2.0、Pinia 3.x
- **新增** TypeScript 依赖
...
```mermaid
graph LR
P["package.json 依赖"] --> V["Vue3 3.5.29"]
P --> R["Vue Router 5.0.3"]
P --> N["Pinia 3.x"]
P --> E["Element Plus 2.13.2"]
P --> K["qiankun"]
T["现代工具链"] --> V35["Vue 3.5.29"]
T --> VR503["Vue Router 5.0.3"]
T --> EP2132["Element Plus 2.13.2"]
T --> V731["Vite 7.3.1"]
T --> TS593["TypeScript 5.9.3"]
T --> VTS220["Vue TSC 2.2.0"]
L["ESLint 依赖"] --> ESL["eslint 8.57.0"]
L --> ESLV["eslint-plugin-vue 9.22.0"]
...
- [packages/main-app/.eslintrc.cjs:8-22](file://packages/main-app/.eslintrc.cjs#L8-L22)
- [packages/main-app/src/main.js:1-49](file://packages/main-app/src/main.js#L1-L49)
- [packages/main-app/src/core/microAppManager.js:1-34](file://packages/main-app/src/core/microAppManager.js#L1-L34)
- [packages/main-app/src/config/microApps.js:1-36](file://packages/main-app/src/config/microApps.js#L1-L36)
...
- **新增** 现代化工具链性能优化
  - Vue 3.5.29 的响应式系统优化和编译器改进
  - Vite 7.3.1 的更快开发服务器和构建性能
  - TypeScript 5.9.3 的增强类型检查和编译性能
  - Pinia 3.x 的性能优化和更好的 TypeScript 支持
- **新增** TypeScript 优化
  - 通过 noEmit 配置避免重复编译，提升构建性能
  - 使用 bundler 模块解析，优化模块打包效率
- **新增** Pinia 3.x 优化
  - Composition API 风格提供更好的性能和开发体验
  - 持久化插件优化存储性能

**章节来源**
- [packages/main-app/vite.config.js:32-44](file://packages/main-app/vite.config.js#L32-L44)
...
- **新增** 现代化工具链相关问题
  - Vue 3.5.29 兼容性：确保子应用与最新 Vue 版本兼容
  - Vue Router 5.0.3：检查路由守卫和导航解析的兼容性
  - Element Plus 2.13.2：注意组件 API 变更和样式更新
  - Vite 7.3.1：检查插件兼容性和构建配置
  - Pinia 3.x：检查 Store 定义和持久化配置
- **新增** TypeScript 相关问题
...
- **新增** Pinia 状态管理问题
  - Store 无法访问：检查 Store 导出和导入路径
  - 持久化失效：确认持久化插件配置和存储键值
  - 状态不同步：检查 Store 的响应式更新和组件订阅

**章节来源**
- [packages/main-app/src/core/microAppManager.js:52-63](file://packages/main-app/src/core/microAppManager.js#L52-L63)
- [packages/main-app/src/core/microAppManager.js:184-196](file://packages/main-app/src/core/microAppManager.js#L184-L196)
- [packages/main-app/src/core/microAppManager.js:254-273](file://packages/main-app/src/core/microAppManager.js#L254-L273)
- [packages/main-app/src/core/microAppManager.js:324-345](file://packages/main-app/src/core/microAppManager.js#L324-L345)
- [packages/main-app/src/core/microAppManager.js:424-447](file://packages/main-app/src/core/microAppManager.js#L424-L447)
- [packages/main-app/src/core/microAppManager.js:396-407](file://packages/main-app/src/core/microAppManager.js#L396-L407)
- [packages/main-app/tsconfig.json:1-46](file://packages/main-app/tsconfig.json#L1-L46)
...
## 结论
本指南基于仓库实际实现，系统梳理了主应用初始化、qiankun 集成、微应用配置与生命周期管理、路由与状态协同、跨应用通信、性能优化与故障排除等关键环节。**更新** 本版本反映了现代化工具链的重大升级，包括 Vue 3.5.29、Vue Router 5.0.3、Element Plus 2.13.2、Vite 7.3.1、Pinia 3.x 等最新版本，显著提升了框架性能、类型安全性和开发效率。开发服务器经过优化，提供了更好的开发体验和跨域支持。Pinia 3.x 状态管理配合持久化插件，提供了更强大的状态管理能力。建议在开发中遵循配置集中化、加载流程标准化、样式隔离与清理规范化、错误日志可追溯、类型安全和代码质量检查以及开发服务器最佳实践的原则，以获得稳定可靠的微前端主应用体验。
...
- 调试入口
  ...
  - 全局访问桥接模块：window.__ARTISAN_BRIDGE__
- **新增** 现代化工具链版本
  - Vue：3.5.29
  - Vue Router：5.0.3
  - Element Plus：2.13.2
...
- **新增** 开发服务器配置
  ...
  - 自定义头部：Access-Control-Allow-Origin: *
- **新增** Pinia 状态管理配置
  - Store 导出：useAppStore、useUserStore
  - 持久化插件：pinia-plugin-persistedstate
  - 存储键值：artisan-app、artisan-user

**章节来源**
- [packages/main-app/package.json:6-13](file://packages/main-app/package.json#L6-L13)
- [packages/main-app/vite.config.js:32-45](file://packages/main-app/vite.config.js#L32-L45)
- [packages/main-app/src/main.js:48-49](file://packages/main-app/src/main.js#L48-L49)
- [packages/main-app/tsconfig.json:1-46](file://packages/main-app/tsconfig.json#L1-L46)
- [packages/main-app/.eslintrc.cjs:1-56](file://packages/main-app/.eslintrc.cjs#L1-L56)
- [packages/main-app/src/env.d.ts:1-29](file://packages/main-app/src/env.d.ts#L1-L29)