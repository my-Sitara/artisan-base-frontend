...
- **动态Origin模式匹配**：localhost和域名后缀匹配的高级特性
...
- **动态Origin模式匹配**：支持localhost和域名后缀的正则表达式匹配，增强跨域治理灵活性
- iframe 加载器（IframeLoader）：负责 iframe 的加载、沙箱、高度自适应、心跳检测与卸载清理
- 微应用实例管理器（MicroAppManager）：多应用实例加载、lastModified 热更新检测、预加载、上下线控制
...
## 架构总览
平台整体由"主应用 + 多类型子应用"构成，主应用通过 qiankun 管理子应用生命周期，通过 Bridge 实现跨应用通信；iframe 子应用通过沙箱与 postMessage 协议进行安全交互。**新增的动态Origin模式匹配为跨域治理提供了更灵活的配置方式**。

```mermaid
graph TB
MainApp["主应用<br/>packages/main-app"] --> QK["qiankun<br/>loadMicroApp"]
MainApp --> Bridge["Bridge<br/>postMessage 通信"]
MainApp --> DynamicOrigin["动态Origin模式匹配<br/>localhost & 域名后缀匹配"]
...
Bridge --> Vue3Sub
Bridge --> Vue2Sub
DynamicOrigin --> Bridge
DynamicOrigin --> IframeLoader
```
...
- 安全策略
  ...
  - **新增动态Origin模式匹配**：支持localhost和域名后缀的正则表达式匹配
...
### 组件B：动态Origin模式匹配高级特性
**新增功能**：平台现在支持动态Origin模式匹配，提供更灵活的跨域治理能力。

- **localhost 匹配规则**
  - 正则表达式：`/^https?:\/\/localhost(:\d+)?$/`
  - 支持 http 和 https 协议
  - 支持可选端口号匹配
  - 适用于本地开发环境的多端口场景

- **域名后缀匹配规则**
  - 正则表达式：`/\.hon\.ide\.dev\.jh/`
  - 支持任意前缀的域名匹配
  - 适用于云IDE等开发环境的统一域名后缀
  - 支持后续路径的任意内容

- **动态origin自动添加**
  - 自动检测并添加当前页面的 window.location.origin
  - 避免重复配置，提升开发体验
  - 支持多环境自动适配
...
```mermaid
flowchart TD
Start(["Origin验证开始"]) --> CheckStatic["检查静态白名单"]
CheckStatic --> CheckCurrent["检查当前页面origin"]
CheckCurrent --> CheckPatterns["检查动态模式匹配"]
CheckPatterns --> Pattern1["localhost匹配<br/>/^https?:\\/\\/localhost(:\\d+)?$/"]
CheckPatterns --> Pattern2["域名后缀匹配<br/>/\\.hon\\.ide\\.dev\\.jh/"]
Pattern1 --> Result["验证结果"]
Pattern2 --> Result
Result --> End(["允许或拒绝"])
```

图示来源
- [packages/main-app/src/core/bridge.js](file://packages/main-app/src/core/bridge.js#L19-L30)
...
### 组件D：微应用实例管理与预加载
- 实例管理
  - 支持多应用实例加载、卸载、刷新；记录实例映射与错误日志；提供实例计数与状态查询
- 预加载
...
- 安全
  - 维护允许的 origin 列表
  - **支持动态Origin模式匹配**：localhost和域名后缀匹配
...
- 运行时优化
  ...
  - **动态Origin模式匹配优化**：减少重复的origin配置，提升验证效率
...
## 结论
本指南从安全、类型系统、性能与扩展四个维度，系统梳理了平台的高级能力与最佳实践。通过严格的跨域治理与沙箱策略保障 iframe 安全，**新增的动态Origin模式匹配为localhost和域名后缀场景提供了灵活的配置方案**，借助 TypeScript 提升开发质量与可维护性，结合预加载与热更新优化用户体验，配合完善的日志与监控体系实现稳定运维。高级用户可在此基础上进一步定制插件、组件与第三方集成方案，充分发挥平台的扩展潜力。
